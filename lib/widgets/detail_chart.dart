...
class DetailChart extends StatefulWidget {
  ...
  const DetailChart({super.key, required this.gameData});
...
class _DetailChartState extends State<DetailChart> {
  ...
  Widget build(BuildContext context) {
    ...
    // Ordena los jugadores por su puntuación final (último valor de totalScore)
    players.sort((a, b) {
      final aScores = List<dynamic>.from(totalScore[a] ?? []);
      final bScores = List<dynamic>.from(totalScore[b] ?? []);
      final aPoints = aScores.isNotEmpty ? (aScores.last as int) : 0;
      final bPoints = bScores.isNotEmpty ? (bScores.last as int) : 0;
      return bPoints.compareTo(aPoints); // Descendente
    });
